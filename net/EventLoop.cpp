...
bool EventLoop::wakeup()
{
    ...
    if (n != sizeof one)
    {
        ...
        LOG_SYSERROR("EventLoop::wakeup() writes %d  bytes instead of 8, fd: %d, error: %d", n, m_wakeupFdSend, (int32_t)error);
#else
        int error = errno;
        LOG_SYSERROR("EventLoop::wakeup() writes %d  bytes instead of 8, fd: %d, error: %d, errorinfo: %s", n, m_wakeupFd, error, strerror(error));
#endif
